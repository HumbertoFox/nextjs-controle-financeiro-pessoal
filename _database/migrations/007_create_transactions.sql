...
CREATE TABLE IF NOT EXISTS transactions (
    ...
    installment_group_id    UUID               NULL,                                -- Grupo de parcelas (para transações parceladas)
    installment_number      SMALLINT           NULL CHECK (installment_number > 0), -- Número da parcela
    installments_total      SMALLINT           NULL CHECK (installments_total > 0), -- Total de parcelas
...
COMMENT ON COLUMN transactions.installment_group_id IS 'Agrupa todas as parcelas de um mesmo lançamento parcelado (NULL = lançamento avulso)';
COMMENT ON COLUMN transactions.installment_number   IS 'Número da parcela atual (1, 2, 3...)';
COMMENT ON COLUMN transactions.installments_total   IS 'Total de parcelas do lançamento';
